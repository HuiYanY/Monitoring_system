...
std::mutex YOLO_V11::mutex;  // 保护实例创建和销毁的互斥锁
...
char* YOLO_V11::CreateSession(DL_INIT_PARAM& iParams) {
    // 初始化返回值为RET_OK，表示成功
    char* Ret = RET_OK;
    static char error[2][100] = {{"[YOLO_V11]:Your model path is error.Change your model path without chinese characters."},{"[YOLO_V11]:Create session failed."}};
...
char* YOLO_V11::TensorProcess(clock_t& starttime_1, cv::Mat& iImg, N& blob, std::vector<int64_t>& inputNodeDims,
    ...
    default:
        std::cout << "[YOLO_V11]: " << "Not support model type." << std::endl;
    }
    return RET_OK;
